...
class CPE:
    ...
    def _getX509Template(self, content):
        ...
        signature = etree.SubElement(content, tag.text, Id='signatureFlexxoone', nsmap={'ds': tag.namespace})
    ...
    def _getDocumentDetail21(self, invoice_id):
        ...
        etree.SubElement(self._root, tag.text, nsmap={'cbc': tag.namespace}).text = invoice_id.name or ''
        ...
        if invoice_id.pe_invoice_code in ('01', '03'):
            ...
            if invoice_id.env.context.get('despatch_numbers') and invoice_id.env.context.get('despatch_numbers', {}).get(invoice_id.id):
    ...
    def _getCompany21(self, invoice_id):
        ...
        tag = etree.QName(self._cbc, 'ID')
        etree.SubElement(party_identification, tag.text, schemeID=invoice_id.company_id.partner_id.l10n_latam_identification_type_id.l10n_pe_vat_code, schemeName='Documento de Identidad',
        ...
        tag = etree.QName(self._cbc, 'AddressTypeCode')
        pe_branch_code = invoice_id.user_id.pe_branch_id.pe_branch_code or invoice_id.company_id.pe_branch_code or "0000"
    ...
    def _getDiscrepancyResponse21(self, invoice_id):
        for inv in invoice_id.pe_related_ids:
            ...
            if invoice_id.type == 'out_invoice':
                etree.SubElement(discrepancy, tag.text, listAgencyName='PE:SUNAT', listURI='urn:pe:gob:sunat:cpe:segem:catalogos:catalogo10', nsmap={'cbc': tag.namespace}).text = invoice_id.pe_debit_note_code
            else:
                if invoice_id.type == 'out_refund':
                    etree.SubElement(discrepancy, tag.text, listAgencyName='PE:SUNAT', listURI='urn:pe:gob:sunat:cpe:segem:catalogos:catalogo09', nsmap={'cbc': tag.namespace}).text = invoice_id.pe_credit_note_code
            tag = etree.QName(self._cbc, 'Description')
            etree.SubElement(discrepancy, tag.text, nsmap={'cbc': tag.namespace}).text = etree.CDATA(invoice_id.name or invoice_id.invoice_line_ids[0].name or '')
    ...
    def _getBillingReference(self, invoice_id, line=None):
        for inv in invoice_id.pe_related_ids:
            ...
            tag = etree.QName(self._cbc, 'ID')
            etree.SubElement(invoice, tag.text, nsmap={'cbc': tag.namespace}).text = inv.name
            tag = etree.QName(self._cbc, 'DocumentTypeCode')
    ...
    def _getPrepaidPayment(self, invoice_id):
        for line in invoice_id.invoice_line_ids.mapped('sale_line_ids').mapped('order_id').mapped('invoice_ids').filtered(lambda inv: inv.pe_sunat_transaction in ('04', ) and inv.pe_invoice_code in ('01', '03') and inv.id != invoice_id.id and inv.state not in ('draft',
            ...
            if line.pe_invoice_code == '01':
                etree.SubElement(prepaid, tag.text, schemeID='02', nsmap={'cbc': tag.namespace}).text = line.name
            else:
                if line.pe_invoice_code == '03':
                    etree.SubElement(prepaid, tag.text, schemeID='03', nsmap={'cbc': tag.namespace}).text = line.name
    ...
    def _getTaxTotal(self, invoice_id):
        amount_tax = invoice_id.currency_id.round(invoice_id.pe_amount_tax * (1 - (invoice_id.discount_rate or 0.0) / 100))
        for tax in invoice_id.tax_line_ids.filtered(lambda tax: tax.tax_id.pe_tax_type.code in ('1000', '2000', '9999') and tax.tax_id.pe_is_charge == False):
            ...
            etree.SubElement(total, tag.text, currencyID=invoice_id.currency_id.name, nsmap={'cbc': tag.namespace}).text = str(round(tax.amount, 2))
        ...
        if not invoice_id.tax_line_ids.filtered(lambda tax: tax.tax_id.pe_tax_type.code == '1000' and tax.tax_id.pe_is_charge == False):
            tag = etree.QName(self._cac, 'TaxTotal')
            total = etree.SubElement(self._root, tag.text, nsmap={'cac': tag.namespace})
            tag = etree.QName(self._cbc, 'TaxAmount')
            etree.SubElement(total, tag.text, currencyID=invoice_id.currency_id.name, nsmap={'cbc': tag.namespace}).text = str(0.0)
            tag = etree.QName(self._cac, 'TaxSubtotal')
            tax_subtotal = etree.SubElement(total, tag.text, nsmap={'cac': tag.namespace})
            tag = etree.QName(self._cbc, 'TaxAmount')
            etree.SubElement(tax_subtotal, tag.text, currencyID=invoice_id.currency_id.name, nsmap={'cbc': tag.namespace}).text = str(0.0)
            tag = etree.QName(self._cac, 'TaxCategory')
            category = etree.SubElement(tax_subtotal, tag.text, nsmap={'cac': tag.namespace})
            tag = etree.QName(self._cac, 'TaxScheme')
            scheme = etree.SubElement(category, tag.text, nsmap={'cac': tag.namespace})
    ...
    def _getTaxTotal21(self, invoice_id):
        ...
        amount_tax = invoice_id.pe_amount_tax #Ya no se esta usando en este metodo Odoo V.13
        ...
        for group_tax in invoice_id.amount_by_group:
            ...
            tag = etree.QName(self._cbc, 'ID')
            etree.SubElement(
                scheme, tag.text, schemeName='Codigo de tributos', schemeAgencyName='PE:SUNAT',
                schemeURI='urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo05',
                nsmap={'cbc': tag.namespace}).text = tax.l10n_pe_edi_tax_code
            tag = etree.QName(self._cbc, 'Name')
        ...
        for tax in tax_ids.filtered(lambda tax: tax.l10n_pe_edi_tax_code != '9996' and tax.pe_is_charge == False):
            ...
            tag = etree.QName(self._cbc, 'TaxableAmount')
            etree.SubElement(
                tax_subtotal, tag.text, currencyID=invoice_id.currency_id.name,
            ...
            tag = etree.QName(self._cbc, 'ID')
            etree.SubElement(
                scheme, tag.text, schemeName='Codigo de tributos',
    ...
    def _getLegalMonetaryTotal21(self, invoice_id):
        tagname = (
            ...
            and 'RequestedMonetaryTotal' or 'LegalMonetaryTotal')
    ...
    def _getDocumentLines21(self, invoice_id):
        ...
        for line in invoice_id.invoice_line_ids.filtered(lambda ln: ln.price_subtotal >= 0):
            ...
            if line.discount > 0 and line.discount < 100 and invoice_id.pe_invoice_code not in ('07',
                                                                                                '08'):
                tag = etree.QName(self._cac, 'AllowanceCharge')
            ...
            if line.pe_charge_amount > 0 and invoice_id.pe_invoice_code not in ('07',
                                                                                '08'):
                tag = etree.QName(self._cac, 'AllowanceCharge')
                charge = etree.SubElement(inv_line, tag.text, nsmap={'cac': tag.namespace})
                tag = etree.QName(self._cbc, 'ChargeIndicator')
            ...
            for tax in line.tax_ids.filtered(lambda tax: tax.pe_is_charge == False):
                if tax.l10n_pe_edi_tax_code == '9996':
                    ...
                    if tax.pe_tax_type.code == '2000':
                        tag = etree.QName(self._cbc, 'TierRange')
                        etree.SubElement(category, tag.text, nsmap={'cbc': tag.namespace}).text = line.pe_tier_range or tax.pe_tier_range
                    elif tax.pe_tax_type.code == '1016': #Agregado por Freddy - todo este bloque
                        tag = etree.QName(self._cbc, 'TaxExemptionReasonCode')
    ...
    def getVoidedDocuments(self, batch):
        ...
        tag = etree.QName(self._cbc, 'IssueDate')
        etree.SubElement(self._root, tag.text, nsmap={'cbc': tag.namespace}).text = batch.send_date.strftime('%Y-%m-%d')
        ...
        for invoice_id in batch.voided_ids:
            if invoice_id.pe_cpe_id.state in ('draft', 'generate', 'cancel'):
                raise UserError(_('The invoice N° %s must be sent to the sunat to generate this document.') % invoice_id.name)
            ...
            etree.SubElement(line, tag.text, nsmap={'sac': tag.namespace}).text = invoice_id.name and invoice_id.name.split('-')[0]
            tag = etree.QName(self._sac, 'DocumentNumberID')
            etree.SubElement(line, tag.text, nsmap={'sac': tag.namespace}).text = invoice_id.name and invoice_id.name.split('-')[1]
    ...
    def getSummaryDocuments(self, batch):
        ...
        for journal_id in journal_ids:
            ...
            for invoice_id in batch.summary_ids.filtered(lambda inv: inv.journal_id.id == journal_id.id).sorted(key=lambda r: r.name):
                ...
                tag = etree.QName(self._cbc, 'ID')
                etree.SubElement(line, tag.text, nsmap={'cbc': tag.namespace}).text = invoice_id.name
